...
export default function FeaturedBlogs({ worldNews }) {
    ...
    return (
        ...
        </section>: worldNews.map((blog , indx) => (
            <section key={indx} className="flex flex-row py-6 mx-2 gap-1 border-b border-gray-700">
            <div className="w-60">
                <p className="text-gray-600">{blog.category || "World"}</p>
                <p className="text-xl font-medium line-clamp-2">{blog.title}</p>
                <p className="text-gray-600 line-clamp-3">{blog.text}</p>
